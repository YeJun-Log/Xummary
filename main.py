...
def summarize_text(tweet_data_list):
    ...
    제공된 트윗 텍스트들과 첨부된 이미지들을 상세히 모두 분석해서 전문가 수준의 '인텔리전스 보고서'를 작성해줘.

    [분석 지침]
    1. 주요 이슈(Headline)를 선정해서 알려줘.
    2. 시각적 분석: 첨부된 이미지가 '차트'라면 수치를 해석하고, '현장 사진'이라면 상황을 텍스트와 연결해 분석해줘. '지도'라면 해당 내용을 국제 정세와 연결해서 잘 설명해줘. (이미지가 없다면 이 항목은 제외할 것)
    3. 대립 의견(Dissenting Voices): 전문가들 사이에서 의견이 갈리는 맥락이 있다면, 반드시 포착해줘.
    4. 향후 전망: 이 정보들이 향후 시장이나 정책에 줄 영향과 주의할 리스크를 정리해줘.
    5. 해당 트윗들의 분야 (정치, 경제 등)에 따라 상세하게 분석해서 알려줘.

    [출력 형식]
    - 작성자, 분석관 이름은 'Gemini' 로 할 것.
    - 누구나 쉽게 알아들을 수 있도록, 쉬운 말로 풀어서 설명할 것. 가능하면 예시를 적극 활용할 것.
    - 모든 답변은 한국어로 작성하고, 중요한 단어는 [](대괄호) 사용할 것.
    - 요약 시, 누가 작성한 글인지 간략한 출처를 남길 것. 
    - 절대 거짓말을 하지 않을 것. 답변 제공 전 근거가 명확한지 검토할 것.
    - 마지막에 '오늘의 한 줄 인사이트' 남길 것.
    """)
